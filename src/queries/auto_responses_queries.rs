...
impl DefaultSeaQueries for AutoResponsesQueries {
    ...
    async fn create_entity(
        ...
        let message = if let Some(data) = create_data.response_data {
            Some(MessageQueries::create_entity(db, data).await?)
        } else {
            None
        };
        ...
        let active_model = Self::ActiveModel {
            ...
            response_id: Set(message.map(|msg| msg.id)),
            ..Default::default()
        };
    ...
    async fn apply_updates(
        ...
        if let Some(message_data) = update_data.response_data {
            if let ActiveValue::Unchanged(Some(message_id)) = active_model.response_id {
                ...
                active_model.response_id = Set(Some(message.id));
            }
        }
