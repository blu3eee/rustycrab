...
impl TicketPanelsQueries {
    ...
    pub async fn fetch_panel_details(
        ...
        let message: ResponseMessageDetails = MessageQueries::fetch_message_response(
            db,
            panel.message_id
        ).await?;

        let button: ResponseButton = MessageButtonQueries::find_by_id(
            db,
            panel.button_id
        ).await?.into();

        let welcome_message: ResponseMessageDetails = MessageQueries::fetch_message_response(
            db,
            panel.welcome_message_id
        ).await?;
...
impl DefaultSeaQueries for TicketPanelsQueries {
    ...
    async fn create_entity(
        ...
        Self::save_active_model(db, Self::ActiveModel {
            ...
            button_id: Set(button.id),
            welcome_message_id: Set(welcome_message.id),
    ...
    async fn apply_updates(
        ...
        if let Some(data) = update_data.message_data {
            ...
            if let ActiveValue::Unchanged(id) = active_model.message_id {
        ...
        if let Some(data) = update_data.welcome_message_data {
            if let ActiveValue::Unchanged(id) = active_model.welcome_message_id {
                MessageQueries::update_by_id(db, id, data).await?;
            }
        }

        if let Some(data) = update_data.button_data {
            if let ActiveValue::Unchanged(id) = active_model.button_id {
