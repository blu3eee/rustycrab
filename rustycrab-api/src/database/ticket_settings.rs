//! `SeaORM` Entity. Generated by sea-orm-codegen 0.12.4

use sea_orm::entity::prelude::*;

#[derive(Clone, Debug, PartialEq, DeriveEntityModel, Eq)]
#[sea_orm(table_name = "ticket_settings")]
pub struct Model {
    #[sea_orm(primary_key)]
    pub id: i32,
    #[sea_orm(column_name = "perUserTicketLimit")]
    pub per_user_ticket_limit: i32,
    #[sea_orm(column_name = "allowUserToCloseTickets")]
    pub allow_user_to_close_tickets: i8,
    #[sea_orm(column_name = "ticketCloseConfirmation")]
    pub ticket_close_confirmation: i8,
    #[sea_orm(column_name = "ticketNotificationChannel")]
    pub ticket_notification_channel: Option<String>,
    #[sea_orm(column_name = "transcriptsChannel")]
    pub transcripts_channel: Option<String>,
    #[sea_orm(column_name = "threadTicket")]
    pub thread_ticket: i8,
    #[sea_orm(column_name = "archiveCategory")]
    pub archive_category: Option<String>,
    #[sea_orm(column_name = "archiveOverflowCategory")]
    pub archive_overflow_category: Option<String>,
    #[sea_orm(column_name = "botId")]
    pub bot_id: i32,
    #[sea_orm(column_name = "guildId")]
    pub guild_id: i32,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveRelation)]
pub enum Relation {
    #[sea_orm(
        belongs_to = "super::bots::Entity",
        from = "Column::BotId",
        to = "super::bots::Column::Id",
        on_update = "NoAction",
        on_delete = "Cascade"
    )]
    Bots,
    #[sea_orm(
        belongs_to = "super::guild_info::Entity",
        from = "Column::GuildId",
        to = "super::guild_info::Column::Id",
        on_update = "NoAction",
        on_delete = "Cascade"
    )]
    GuildInfo,
}

impl Related<super::bots::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Bots.def()
    }
}

impl Related<super::guild_info::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::GuildInfo.def()
    }
}

impl ActiveModelBehavior for ActiveModel {}
